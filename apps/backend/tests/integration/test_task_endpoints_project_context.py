...
from app.db.models import User, Task, Project, ProjectMember, ProjectRole
import uuid
...
class TestTaskProjectContext:
    ...
    def test_create_task_with_project(self, test_client: TestClient, test_user: User, auth_headers: dict, test_db: Session):
    ...
    def test_create_task_in_unauthorized_project(self, test_client: TestClient, test_user: User, 
                                                 second_user: User, auth_headers: dict, 
                                                 second_auth_headers: dict, test_db: Session):
    ...
    def test_list_tasks_by_project(self, test_client: TestClient, test_user: User, 
                                   auth_headers: dict, test_db: Session):
        ...
        for i in range(2):
            test_client.post(
                "/tasks",
                json={"title": f"Personal Task {i}"},
                headers=auth_headers
            )
        
        # List tasks filtered by project
        response = test_client.get(f"/tasks?project_id={project_id}", headers=auth_headers)
        assert response.status_code == 200
        tasks = response.json()
        assert len(tasks) == 3
        assert all(task["project_id"] == project_id for task in tasks)
    
    def test_get_project_tasks_endpoint(self, test_client: TestClient, test_user: User,
                                       auth_headers: dict, test_db: Session):
        """Test the dedicated project tasks endpoint"""
        # Create a project
        project_response = test_client.post(
            "/projects",
            json={"name": "Project Tasks Test", "description": "Testing project tasks endpoint"},
            headers=auth_headers
        )
        project_id = project_response.json()["id"]
        ...
        response = test_client.get(
            f"/tasks/project/{project_id}?task_status=in_progress",
            headers=auth_headers
        )
        assert response.status_code == 200
        tasks = response.json()
    ...
    def test_update_task_project(self, test_client: TestClient, test_user: User,
                                auth_headers: dict, test_db: Session):
    ...
    def test_project_member_permissions(self, test_client: TestClient, test_user: User,
                                       second_user: User, auth_headers: dict,
                                       second_auth_headers: dict, test_db: Session):
        ...
        project_response = test_client.post(
            "/projects",
            json={"name": "Permission Test Project", "description": "Testing permissions"},
            headers=auth_headers
        )
        project_id = project_response.json()["id"]
    ...
    def test_delete_task_with_project_permissions(self, test_client: TestClient, test_user: User,
                                                  second_user: User, auth_headers: dict,
                                                  second_auth_headers: dict, test_db: Session):
        ...
        project_response = test_client.post(
            "/projects",
            json={"name": "Delete Test Project", "description": "Testing delete permissions"},
            headers=auth_headers
        )
        project_id = project_response.json()["id"]
        ...
        delete_response = test_client.delete(f"/tasks/{task_id}", headers=second_auth_headers)
