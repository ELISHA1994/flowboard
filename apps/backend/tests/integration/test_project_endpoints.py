...
class TestProjectEndpoints:
    ...
    def test_list_projects(
        ...
        assert any(
            p["name"] == "Project 3" and p["user_role"] == "member" for p in data
        )

        # Test including inactive projects
        response = test_client.get(
            "/projects/?include_inactive=true", headers=auth_headers
        )
    ...
    def test_get_project_no_access(
        self,
        test_client: TestClient,
        test_user: User,
        auth_headers: dict,
        ...
        project = Project(
            id=str(uuid.uuid4()), name="Private Project", owner_id=other_user.id
        )
        ...
        assert "Project not found" in response_data.get(
    ...
    def test_update_project_insufficient_permissions(
        ...
        project = Project(
            id=str(uuid.uuid4()), name="Test Project", owner_id=str(uuid.uuid4())
        )
        member = ProjectMember(
            id=str(uuid.uuid4()),
            project_id=project.id,
        ...
        response_data = response.json()
        assert "Insufficient permissions" in response_data.get(
            "detail", response_data.get("message", "")
        )

    def test_delete_project_soft(
        self,
        test_client: TestClient,
        test_user: User,
        auth_headers: dict,
    ...
    def test_add_project_member(
        ...
        project = Project(
            id=str(uuid.uuid4()), name="Test Project", owner_id=test_user.id
        )
        new_user = User(
            id=str(uuid.uuid4()),
            username="newmember",
        ...
        member = (
            ...
            .filter_by(project_id=project.id, user_id=new_user.id)
            .first()
        )
    ...
    def test_update_member_role(
        ...
        updated_member = (
            test_db.query(ProjectMember)
            .filter_by(project_id=project.id, user_id=other_user.id)
            .first()
        )
    ...
    def test_remove_project_member(
        ...
        response = test_client.delete(
            f"/projects/{project.id}/members/{other_user.id}", headers=auth_headers
        )
    ...
    def test_member_self_remove(
        ...
        response = test_client.delete(
            f"/projects/{project.id}/members/{test_user.id}", headers=auth_headers
        )
    ...
    def test_create_invitation(
        ...
        invitation_data = {"invitee_email": "newuser@example.com", "role": "member"}
    ...
    def test_accept_invitation(
        ...
        response = test_client.post(
            f"/projects/invitations/{invitation.token}/accept", headers=auth_headers
        )
        ...
        updated_invitation = (
            test_db.query(ProjectInvitation).filter_by(id=invitation.id).first()
        )
    ...
    def test_accept_invitation_wrong_email(
        ...
        response_data = response.json()
        assert "This invitation is not for you" in response_data.get(
            "detail", response_data.get("message", "")
        )
    ...
    def test_project_permissions(
        ...
        assert (
            project.has_permission(test_user.id, ProjectRole.VIEWER) is True
