...
from app.db.models import (Category, SavedSearch, Tag, Task, TaskPriority,
                           TaskStatus, User)
...
class TestSearchEndpoints:
    ...
    def test_search_tasks_basic(
        ...
        response = test_client.post(
            "/search/tasks", headers=auth_headers, json={"text": "search"}
        )
        ...
        response = test_client.post(
            ...
            json={"filters": [{"field": "status", "operator": "eq", "value": "todo"}]},
        )
        assert response.status_code == 200
        data = response.json()
        assert data["total"] == 1
    ...
    def test_search_suggestions(
        ...
        # Create test data
        category = Category(
            id="cat1", name="Work", color="#0000FF", user_id=test_user.id
        )
        tag = Tag(id="tag1", name="urgent", color="#FF0000", user_id=test_user.id)
        test_db.add_all([category, tag])
        test_db.commit()

        response = test_client.get("/search/suggestions", headers=auth_headers)
        assert response.status_code == 200
        data = response.json()
    ...
    def test_saved_search_crud(
        ...
        response = test_client.post(
            "/search/saved", headers=auth_headers, json=search_data
        )
        assert response.status_code == 200
        ...
        # Get saved searches
        response = test_client.get("/search/saved", headers=auth_headers)
        assert response.status_code == 200
        ...
        response = test_client.get(f"/search/saved/{search_id}", headers=auth_headers)
        assert response.status_code == 200
        assert response.json()["id"] == search_id

        # Update saved search
        update_data = {"name": "Updated Search Name", "is_default": False}
        response = test_client.put(
            f"/search/saved/{search_id}", headers=auth_headers, json=update_data
        )
        assert response.status_code == 200
        ...
        response = test_client.delete(
            f"/search/saved/{search_id}", headers=auth_headers
        )
        assert response.status_code == 200
    ...
    def test_bulk_operations(
        ...
        response = test_client.post(
            "/search/bulk",
            headers=auth_headers,
            json={"task_ids": [task_ids[0]], "operation": "delete"},
        )
        assert response.status_code == 200
        result = response.json()
    ...
    def test_bulk_operations_with_tags(
        ...
        response = test_client.post(
            "/search/bulk",
            headers=auth_headers,
            json={"task_ids": ["t1"], "operation": "remove_tags", "value": ["review"]},
        )
        assert response.status_code == 200
    ...
    def test_search_pagination(
        ...
        # Test first page
        response = test_client.post(
            "/search/tasks", headers=auth_headers, json={"skip": 0, "limit": 10}
        )
        assert response.status_code == 200
        data = response.json()
        ...
        # Test second page
        response = test_client.post(
            "/search/tasks", headers=auth_headers, json={"skip": 10, "limit": 10}
        )
        assert response.status_code == 200
        data = response.json()
        assert data["total"] == 25
        ...
        # Test last page
        response = test_client.post(
            "/search/tasks", headers=auth_headers, json={"skip": 20, "limit": 10}
        )
        assert response.status_code == 200
        data = response.json()
        assert data["total"] == 25
    ...
    def test_invalid_search_operations(
        ...
        response = test_client.post(
            "/search/bulk",
            headers=auth_headers,
            json={"task_ids": ["task1"], "operation": "invalid_operation"},
        )
        assert response.status_code == 422
