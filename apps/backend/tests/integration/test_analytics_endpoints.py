...
from app.db.models import Task, TaskStatus, TaskPriority, Category, Tag, User, Project, TimeLog, ProjectRole
...
class TestAnalyticsEndpoints:
    ...
    def test_get_productivity_trends(
        ...
        for i in range(1, 4):
            assert data["trends"][i]["period_start"] > data["trends"][i-1]["period_start"]
    ...
    def test_category_distribution(
        ...
        cat2 = Category(id="cat2", name="Personal", color="#00FF00", user_id=test_user.id)
    ...
    def test_team_performance_report(
        ...
        # Add team member
        ProjectService.add_member(test_db, project.id, second_user.id, ProjectRole.MEMBER)
        ...
        response = test_client.get(
            f"/analytics/team-performance/{project.id}",
            headers=auth_headers
        )
        assert response.status_code == 200
        data = response.json()
        ...
        # Check owner stats
        owner_stats = next(m for m in data["team_members"] if m["user_id"] == test_user.id)
        ...
        # Check member stats
        member_stats = next(m for m in data["team_members"] if m["user_id"] == second_user.id)
