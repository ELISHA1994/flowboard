...
from app.core.exceptions import (BadRequestException, ConflictException,
                                 NotFoundException)
from app.db.models import Task, TaskDependency, TaskPriority, TaskStatus
...
class TestTaskDependencyService:
    ...
    def test_create_dependency_task_not_found(
        self, mock_db, sample_tasks, sample_user_id
    ):
    ...
    def test_create_dependency_already_exists(
        self, mock_db, sample_tasks, sample_user_id
    ):
        """Test dependency creation when it already exists"""
        task1, task2, _ = sample_tasks
        existing_dependency = TaskDependency(
            id=str(uuid.uuid4()), task_id=task1.id, depends_on_id=task2.id
        )
    ...
    def test_create_dependency_self_reference(
        self, mock_db, sample_tasks, sample_user_id
    ):
    ...
    def test_create_dependency_circular_reference(
        self, mock_cycle_check, mock_db, sample_tasks, sample_user_id
    ):
    ...
    def test_delete_dependency_success(self, mock_db, sample_user_id):
        ...
        dependency = TaskDependency(
            id=str(uuid.uuid4()), task_id="task1", depends_on_id="task2"
        )
    ...
    def test_get_task_dependencies(self, mock_db, sample_tasks, sample_user_id):
        ...
        dep2 = TaskDependency(
            id=str(uuid.uuid4()), task_id=task1.id, depends_on_id=task3.id
        )
    ...
    def test_can_complete_task_with_incomplete_dependencies(
        self, mock_db, sample_tasks, sample_user_id
    ):
    ...
    def test_can_complete_task_with_completed_dependencies(
        self, mock_db, sample_tasks, sample_user_id
    ):
    ...
    def test_update_task_parent_remove_parent(
        self, mock_db, sample_tasks, sample_user_id
    ):
    ...
    def test_update_task_parent_self_parent(
        self, mock_db, sample_tasks, sample_user_id
    ):
    ...
    def test_update_task_parent_circular_hierarchy(
        self, mock_circular_check, mock_db, sample_tasks, sample_user_id
    ):
    ...
    def test_update_parent_task_status_all_done(self, mock_db):
        ...
        parent_task = Task(id="parent", status=TaskStatus.IN_PROGRESS)
    ...
    def test_update_parent_task_status_any_in_progress(self, mock_db):
        ...
        parent_task = Task(id="parent", status=TaskStatus.TODO)
        subtask1 = Task(id="sub1", parent_task_id="parent", status=TaskStatus.DONE)
        subtask2 = Task(
            id="sub2", parent_task_id="parent", status=TaskStatus.IN_PROGRESS
        )
    ...
    def test_update_parent_task_status_all_todo(self, mock_db):
        ...
        parent_task = Task(
            id="parent", status=TaskStatus.DONE, completed_at="2023-01-01"
        )
