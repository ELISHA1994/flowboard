...
class TestCompleteUserJourney:
    ...
    def test_new_user_complete_workflow(
        ...
        update_response = test_client.put(
            f"/tasks/{task_to_update['id']}", json=update_data, headers=headers
        )
        assert update_response.status_code == 200
        assert update_response.json()["status"] == "in_progress"

        # Step 7: Filter tasks by status
        in_progress_response = test_client.get(
            "/tasks/?task_status=in_progress", headers=headers
        )
        ...
        complete_response = test_client.put(
            f"/tasks/{task_to_update['id']}", json={"status": "done"}, headers=headers
        )
        ...
        delete_response = test_client.delete(
            f"/tasks/{task_to_delete['id']}", headers=headers
        )
    ...
    def test_user_isolation_workflow(self, test_client: TestClient, test_db: Session):
        ...
        for i in range(3):
            ...
            response = test_client.post(
                "/tasks/", json=task_data, headers=alice_headers
            )
        ...
        response = test_client.put(
            f"/tasks/{bob_task_id}", json={"title": "Hacked!"}, headers=alice_headers
        )
        assert response.status_code == 403
...
class TestErrorHandlingWorkflow:
    ...
    def test_concurrent_updates_workflow(
        self, test_client: TestClient, test_user: User, test_user_token: str
    ):
...
class TestPaginationWorkflow:
    ...
    def test_large_dataset_pagination(
        ...
        todo_tasks = test_client.get(
            "/tasks/?task_status=todo&limit=5", headers=headers
        )
