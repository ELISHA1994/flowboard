...
from app.db.models import User, Task, Notification, TaskReminder
from app.core.middleware.jwt_auth_backend import get_current_user
from app.models.notification import (
    NotificationResponse, NotificationMarkRead, NotificationPreferenceUpdate,
    NotificationPreferenceResponse, TaskReminderCreate, TaskReminderUpdate,
    TaskReminderResponse, TaskDueDateRemindersCreate, NotificationStats
)
...
async def mark_multiple_notifications_read(
    ...
    for notification_id in mark_read.notification_ids:
        if NotificationService.mark_notification_read(db, notification_id, current_user.id):
            count += 1
...
async def mark_all_notifications_read(
    notification_type: Optional[str] = Query(None, description="Only mark specific type as read"),
    current_user: User = Depends(get_current_user),
    db: Session = Depends(get_db)
):
...
async def delete_task_reminder(
    ...
    success = NotificationService.delete_task_reminder(
        db,
        reminder_id,
        current_user.id
    )
