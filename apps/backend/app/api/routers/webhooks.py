...
from app.models.webhook import (
    ...
    WebhookSubscriptionResponse, WebhookDeliveryResponse, WebhookTestRequest
)
...
async def delete_webhook_subscription(
    ...
    success = WebhookService.delete_subscription(
        db,
        subscription_id,
        current_user.id
    )
...
async def test_webhook(
    ...
    async def send_test_webhook():
        ...
        if test_request.secret:
            ...
            signature = hmac.new(
                test_request.secret.encode('utf-8'),
                payload_bytes,
                hashlib.sha256
            ).hexdigest()
        ...
        try:
            ...
            logger.info(f"Test webhook sent to {test_request.url}: {response.status_code}")
