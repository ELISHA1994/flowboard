...
from app.db.models import ActivityType, Project, ProjectRole
...
from app.tasks.activities import (log_assignment_change_async,
                                  log_description_change_async,
                                  log_due_date_change_async,
                                  log_priority_change_async,
                                  log_subtask_added_async,
                                  log_task_completed_async,
                                  log_time_logged_async,
                                  log_title_change_async, queue_activity_log,
                                  queue_status_change_log,
                                  queue_task_creation_log)
...
def format_task_response(task: TaskModel, include_subtasks: bool = False) -> dict:
    ...
    response = {
        ...
        "categories": [
            {"id": c.id, "name": c.name, "color": c.color} for c in task.categories
        ],
        ...
        "recurrence_pattern": (
            task.recurrence_pattern if hasattr(task, "recurrence_pattern") else None
        ),
        "recurrence_interval": (
            task.recurrence_interval if hasattr(task, "recurrence_interval") else None
        ),
        "recurrence_days_of_week": (
            task.recurrence_days_of_week
            if hasattr(task, "recurrence_days_of_week")
            else None
        ),
        "recurrence_day_of_month": (
            task.recurrence_day_of_month
            if hasattr(task, "recurrence_day_of_month")
            else None
        ),
        "recurrence_month_of_year": (
            task.recurrence_month_of_year
            if hasattr(task, "recurrence_month_of_year")
            else None
        ),
        "recurrence_end_date": (
            task.recurrence_end_date if hasattr(task, "recurrence_end_date") else None
        ),
        "recurrence_count": (
            task.recurrence_count if hasattr(task, "recurrence_count") else None
        ),
        "recurrence_parent_id": (
            task.recurrence_parent_id if hasattr(task, "recurrence_parent_id") else None
        ),
    }
...
async def create_task(
    ...
    if task_data.position == 0:
        max_position = (
            db.query(TaskModel).filter(TaskModel.user_id == current_user.id).count()
        )
    ...
    if task_data.assigned_to_id:
        ...
        if task_data.project_id:
            ...
            assigned_user = (
                db.query(User).filter(User.id == task_data.assigned_to_id).first()
            )
    ...
    if task_data.category_ids:
        for category_id in task_data.category_ids:
            CategoryService.add_category_to_task(
                db, db_task.id, category_id, current_user.id
            )
    ...
    # If this is a subtask, also log subtask addition to parent task
    if db_task.parent_task_id:
        log_subtask_added_async.delay(
            task_id=db_task.parent_task_id,  # Log on parent task
            user_id=current_user.id,
            subtask_id=db_task.id,
            subtask_title=db_task.title,
        )
...
async def get_task(
    ...
    if task.user_id != current_user.id:
        ...
        if task.project_id:
            ...
            if not project or not project.has_permission(
                current_user.id, ProjectRole.VIEWER
            ):
                raise HTTPException(
                    status_code=status.HTTP_403_FORBIDDEN,
...
async def update_task(
    ...
    if task.user_id != current_user.id:
        ...
        if task.project_id:
            project = db.query(Project).filter(Project.id == task.project_id).first()
            if not project or not project.has_permission(
                current_user.id, ProjectRole.MEMBER
            ):
                raise HTTPException(
                    status_code=status.HTTP_403_FORBIDDEN,
    ...
    if "category_ids" in update_data:
        ...
        for category_id in update_data["category_ids"]:
            CategoryService.add_category_to_task(
                db, task_id, category_id, current_user.id
            )
    ...
    if "project_id" in update_data:
        if update_data["project_id"] != task.project_id:
            ...
            if task.project_id:
                ...
                if old_project and not old_project.has_permission(
                    current_user.id, ProjectRole.MEMBER
                ):
                    raise HTTPException(
                        status_code=status.HTTP_403_FORBIDDEN,
    ...
    if "assigned_to_id" in update_data:
        if update_data["assigned_to_id"]:
            ...
            if task.project_id:
                ...
                if project:
                    assigned_user_role = project.get_member_role(
    ...
    if "status" in update_data and task.parent_task_id:
        parent_task = (
            db.query(TaskModel).filter(TaskModel.id == task.parent_task_id).first()
        )
        if parent_task:
    ...
    for field, new_value in update_data.items():
        ...
        if field == "status" and old_value != new_value.value:
            # Log status change
            queue_status_change_log(
                task_id=task.id,
                user_id=current_user.id,
                old_status=old_value,
                new_status=new_value.value,
            )

            # Log completion activity if task was completed
            if new_value == TaskStatus.DONE:
                log_task_completed_async.delay(task_id=task.id, user_id=current_user.id)

        elif field == "priority" and old_value != new_value.value:
            # Log priority change
            log_priority_change_async.delay(
                task_id=task.id,
                user_id=current_user.id,
                old_priority=old_value,
                new_priority=new_value.value,
            )

        elif field == "title" and old_value != new_value:
            # Log title change
            log_title_change_async.delay(
                task_id=task.id,
                user_id=current_user.id,
                old_title=old_value,
                new_title=new_value,
        ...
        elif field == "description" and old_value != new_value:
            # Log description change
            log_description_change_async.delay(
                task_id=task.id,
                user_id=current_user.id,
            ...
            new_due_date_str = new_value.isoformat() if new_value else None
            if old_value != new_due_date_str:
                log_due_date_change_async.delay(
                    task_id=task.id,
                    user_id=current_user.id,
                    old_due_date=old_value,
                    new_due_date=new_due_date_str,
                )

        elif field == "assigned_to_id" and old_value != new_value:
            # Log assignment change
            log_assignment_change_async.delay(
                task_id=task.id,
                user_id=current_user.id,
                old_assignee_id=old_value,
...
async def delete_task(
    ...
    queue_activity_log(
        ...
        details={"task_title": task.title, "deletion_reason": "User deleted task"},
    )
...
async def add_task_time(
    ...
    log_time_logged_async.delay(
        task_id=task.id,
        user_id=current_user.id,
        hours=time_update.hours_to_add,
        description=getattr(time_update, "description", None),
    )
...
async def update_task_position(
    ...
    success = TaskService.update_task_positions(
        db, current_user.id, task_id, new_position
    )
    if not success:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
...
async def get_project_tasks(
    ...
    return [
        format_task_response(task, include_subtasks=include_subtasks) for task in tasks
    ]
...
async def share_task(
    ...
    shared_with_user = (
        db.query(User).filter(User.id == share_data.shared_with_id).first()
    )
...
async def get_tasks_shared_with_me(
    ...
    shares = (
        db.query(TaskShare).filter(TaskShare.shared_with_id == current_user.id).all()
    )
...
async def remove_task_share(
    ...
    share = (
        db.query(TaskShare)
        .filter(TaskShare.id == share_id, TaskShare.task_id == task_id)
        .first()
    )

    if not share:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="Task share not found"
        )
...
async def get_recurring_task_instances(
    ...
    if parent_task.user_id != current_user.id:
        if parent_task.project_id:
            project = (
                db.query(Project).filter(Project.id == parent_task.project_id).first()
            )
            if not project or not project.has_permission(
                current_user.id, ProjectRole.VIEWER
            ):
    ...
    instances = (
        db.query(TaskModel)
        .filter(TaskModel.recurrence_parent_id == task_id)
        .order_by(TaskModel.start_date)
        .all()
    )
...
async def get_next_occurrence_date(
    task_id: str,
    reference_date: datetime = Query(
        None, description="Reference date to calculate from (defaults to now)"
    ),
    current_user: UserModel = Depends(get_current_active_user),
    db: Session = Depends(get_db),
):
