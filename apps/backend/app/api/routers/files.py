...
"""
from fastapi import APIRouter, HTTPException, status, Depends, UploadFile, File, Path
from fastapi.responses import FileResponse
from sqlalchemy.orm import Session
from datetime import datetime

from app.db.database import get_db
from app.db.models import Task as TaskModel, User as UserModel, FileAttachment as FileAttachmentModel
from app.models.file_attachment import FileAttachmentResponse, FileAttachmentList, FileUploadLimits
...
@router.post("/tasks/{task_id}/attachments", response_model=FileAttachmentResponse, status_code=status.HTTP_201_CREATED)
async def upload_file_attachment(
    ...
    if task.user_id == current_user.id:
        ...
        for share in task.shares:
            if share.shared_with_id == current_user.id and share.permission in ['edit', 'comment']:
...
async def get_task_attachments(
    ...
    for attachment in attachments:
        attachment_responses.append(FileAttachmentResponse(
            id=attachment.id,
            task_id=attachment.task_id,
            uploaded_by_id=attachment.uploaded_by_id,
            filename=attachment.filename,
...
async def download_attachment(
    ...
    attachment = db.query(FileAttachmentModel).filter(
        FileAttachmentModel.id == attachment_id
    ).first()
