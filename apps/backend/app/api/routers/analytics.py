...
from app.models.analytics import (CategoryDistribution, ExportRequest,
                                  ProductivityTrendsResponse, TagDistribution,
                                  TaskStatistics, TeamPerformanceReport,
...
async def get_productivity_trends(
    ...
    if period not in ["week", "month", "quarter"]:
        raise HTTPException(
            status_code=400, detail="Invalid period. Use week, month, or quarter"
        )
    ...
    trends = AnalyticsService.get_productivity_trends(
        db, current_user.id, period, lookback
    )
    return trends
...
async def get_time_tracking_report(
    ...
    report = AnalyticsService.get_time_tracking_report(
        db, current_user.id, request.start_date, request.end_date, request.group_by
    )
    return report
...
async def get_team_performance(
    ...
    report = AnalyticsService.get_team_performance(db, project_id, current_user.id)
...
async def export_tasks(
    ...
    if export_request.format == "csv":
        content = AnalyticsService.export_tasks_csv(
            db, current_user.id, export_request.task_ids
        )
