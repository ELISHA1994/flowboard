...
class TagService:
    ...
    def create_tag(db: Session, user_id: str, tag: TagCreate) -> Tag:
        ...
        existing = (
            db.query(Tag).filter(Tag.user_id == user_id, Tag.name == tag_name).first()
        )
        ...
        db_tag = Tag(
            id=str(uuid.uuid4()), name=tag_name, color=tag.color, user_id=user_id
        )
    ...
    def get_or_create_tags(
        db: Session, user_id: str, tag_names: List[str]
    ) -> List[Tag]:
    ...
    def get_tag_by_name(db: Session, tag_name: str, user_id: str) -> Optional[Tag]:
        ...
        return (
            db.query(Tag)
            .filter(Tag.name == tag_name.strip().lower(), Tag.user_id == user_id)
            .first()
        )

    @staticmethod
    def update_tag(
        db: Session, tag_id: str, user_id: str, tag_update: TagUpdate
    ) -> Optional[Tag]:
        """Update a tag"""
    ...
    @staticmethod
    def set_task_tags(
        db: Session, task_id: str, tag_names: List[str], user_id: str
    ) -> List[Tag]:
    ...
    def get_popular_tags(db: Session, user_id: str, limit: int = 10) -> List[dict]:
        ...
        for tag in tags:
            ...
            if count > 0:  # Only include used tags
                tag_counts.append({"tag": tag, "task_count": count})
