...
class ProjectService:
    ...
    @staticmethod
    def create_project(
        db: Session, project_data: ProjectCreate, owner_id: str
    ) -> Project:
    ...
    @staticmethod
    def update_project(
        db: Session, project: Project, update_data: ProjectUpdate
    ) -> Project:
    ...
    @staticmethod
    def add_member(
        db: Session, project_id: str, user_id: str, role: ProjectRole
    ) -> ProjectMember:
        ...
        member = ProjectMember(
            id=str(uuid.uuid4()), project_id=project_id, user_id=user_id, role=role
        )
    ...
    def update_member_role(
        db: Session, member: ProjectMember, new_role: ProjectRole
    ) -> ProjectMember:
    ...
    def create_invitation(
        ...
        existing_user = (
            db.query(User).filter(User.email == invitation_data.invitee_email).first()
        )
        if existing_user:
            existing_member = (
                db.query(ProjectMember)
    ...
    def accept_invitation(
        ...
        member = ProjectService.add_member(
            db, invitation.project_id, user.id, invitation.role
        )
    ...
    def to_detail_response(
        project: Project, user_id: str, db: Session
        ...
        return ProjectDetailResponse(**base_response.model_dump(), members=members)
