...
class TaskDependencyService:
    ...
    def create_dependency(
        db: Session, task_id: str, depends_on_id: str, user_id: str
    ) -> TaskDependency:
        """
        ...
        depends_on = (
            db.query(Task)
            .filter(and_(Task.id == depends_on_id, Task.user_id == user_id))
            .first()
        )
        if not depends_on:
        ...
        dependency = TaskDependency(
            id=str(uuid.uuid4()), task_id=task_id, depends_on_id=depends_on_id
        )
        db.add(dependency)
        db.commit()
    ...
    def _would_create_cycle(db: Session, task_id: str, depends_on_id: str) -> bool:
        ...
        def has_path(from_id: str, to_id: str, visited_set: Set[str]) -> bool:
            ...
            dependencies = (
                db.query(TaskDependency).filter(TaskDependency.task_id == from_id).all()
            )
    ...
    @staticmethod
    def delete_dependency(db: Session, dependency_id: str, user_id: str) -> bool:
        """Delete a task dependency"""
        # Find the dependency and verify ownership
        dependency = (
            db.query(TaskDependency)
            .join(Task, TaskDependency.task_id == Task.id)
            .filter(and_(TaskDependency.id == dependency_id, Task.user_id == user_id))
            .first()
        )
    ...
    def can_complete_task(
        ...
        dependencies = (
            ...
            .filter(and_(TaskDependency.task_id == task_id, Task.user_id == user_id))
            .all()
        )
    ...
    def update_task_parent(
        db: Session, task_id: str, parent_task_id: Optional[str], user_id: str
    ) -> Task:
        """
        ...
        parent_task = (
            db.query(Task)
            .filter(and_(Task.id == parent_task_id, Task.user_id == user_id))
            .first()
        )
        if not parent_task:
    ...
    def _would_create_circular_hierarchy(
        db: Session, task_id: str, new_parent_id: str
    ) -> bool:
    ...
    @staticmethod
    def get_subtasks(db: Session, task_id: str, user_id: str) -> List[Task]:
    ...
    @staticmethod
    def update_parent_task_status(db: Session, parent_task: Task) -> None:
        """
        Update parent task status based on subtasks.
        Called after subtask status changes.
        """
        subtasks = db.query(Task).filter(Task.parent_task_id == parent_task.id).all()
