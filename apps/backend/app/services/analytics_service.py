...
from app.db.models import (Category, Project, ProjectRole, Tag, Task,
                           TaskPriority, TaskStatus, TimeLog)
...
class AnalyticsService:
    ...
    def get_task_statistics(
        ...
        overdue_tasks = base_query.filter(
            Task.due_date < datetime.now(timezone.utc), Task.status != TaskStatus.DONE
        ).count()
    ...
    def get_productivity_trends(
        ...
        for i in range(lookback):
            ...
            hours_logged = (
                db.query(func.coalesce(func.sum(TimeLog.hours), 0))
                .filter(
        ...
        return {"period_type": period, "trends": trends}
    ...
    def get_time_tracking_report(
        ...
        if group_by == "task":
            ...
            daily_times = (
                ...
                .group_by(func.date(TimeLog.logged_at))
                .all()
            )

            for date, hours, count in daily_times:
                entries.append(
                    {"date": date, "total_hours": float(hours), "log_count": count}
                )
    ...
    def get_category_distribution(
        ...
        query = (
            ...
            .filter(or_(Task.user_id == user_id, Task.assigned_to_id == user_id))
        )
    ...
    def get_tag_distribution(
        ...
        query = (
            db.query(
                Tag.id, Tag.name, Tag.color, func.count(Task.id).label("task_count")
            )
            .join(Task.tags)
            .filter(or_(Task.user_id == user_id, Task.assigned_to_id == user_id))
        )
        ...
        return [
            {"tag_id": tag_id, "tag_name": name, "color": color, "task_count": count}
            for tag_id, name, color, count in results
        ]

    @staticmethod
    def export_tasks_csv(
        db: Session, user_id: str, task_ids: Optional[List[str]] = None
    ) -> str:
    ...
    def export_tasks_excel(
        ...
        for row, task in enumerate(tasks, 2):
            ...
            ws.cell(
                row=row,
                column=8,
                value=task.completed_at.isoformat() if task.completed_at else "",
            )
            ...
            ws.cell(
                row=row,
                column=11,
                value=", ".join([cat.name for cat in task.categories]),
            )
            ws.cell(
                row=row, column=12, value=", ".join([tag.name for tag in task.tags])
            )
            ws.cell(
                row=row,
                column=13,
                value=task.assigned_to.username if task.assigned_to else "",
            )
    ...
    def get_team_performance(
        ...
        owner_stats["tasks_assigned"] = (
            db.query(func.count(Task.id))
            .filter(
                Task.project_id == project_id, Task.assigned_to_id == project.owner_id
            )
            .scalar()
        )

        # Count owner's completed tasks
        owner_stats["tasks_completed"] = (
            db.query(func.count(Task.id))
            .filter(
        ...
        hours = (
            ...
            .filter(Task.project_id == project_id, TimeLog.user_id == project.owner_id)
            .scalar()
        )
        ...
        for member in project.members:
            ...
            member_stats["tasks_assigned"] = (
                db.query(func.count(Task.id))
                .filter(
                    Task.project_id == project_id, Task.assigned_to_id == member.user_id
                )
                .scalar()
            )

            # Count completed tasks
            member_stats["tasks_completed"] = (
                db.query(func.count(Task.id))
                .filter(
                    Task.project_id == project_id,
            ...
            hours = (
                ...
                .join(Task, TimeLog.task_id == Task.id)
                .filter(
                    Task.project_id == project_id, TimeLog.user_id == member.user_id
                )
                .scalar()
            )
