...
class RecurrenceService:
        ...
        if pattern == RecurrencePattern.DAILY:
            ...
            try:
                return base_date.replace(
                    year=year, month=month_of_year, day=day_of_month
                )
            except ValueError:
    ...
    @staticmethod
    def should_create_next_occurrence(task: Task, current_date: datetime) -> bool:
        """
        ...
        if task.recurrence_count:
            ...
            instance_count = len(
                [t for t in task.recurrence_instances if t.id != task.id]
            )
    ...
    def create_recurring_instance(
        db: Session, parent_task: Task, occurrence_date: datetime
    ) -> Task:
        """
    ...
    def create_task_with_recurrence(
        ...
        if (
            recurrence_config.pattern == RecurrencePattern.WEEKLY
            and recurrence_config.days_of_week
        ):
            task.recurrence_days_of_week = ",".join(
                map(str, recurrence_config.days_of_week)
            )
        elif (
            recurrence_config.pattern == RecurrencePattern.MONTHLY
            and recurrence_config.day_of_month
        ):
    ...
    def update_recurrence(
        db: Session, task: Task, recurrence_config: RecurrenceConfig
    ) -> Task:
        """
    ...
    def delete_recurrence(
        db: Session, task: Task, delete_instances: bool = False
    ) -> None:
        """
    ...
    def get_recurring_tasks_to_process(
        db: Session, current_date: datetime
    ) -> List[Task]:
        """
    ...
    def process_recurring_tasks(db: Session):
        ...
        logger.warning(
            "process_recurring_tasks called directly - this should be handled by Celery periodic task"
        )
    ...
    def should_create_next_instance(db: Session, task_id: str) -> bool:
        ...
        if (
            ...
            and datetime.now(timezone.utc) > task.recurrence_end_date
        ):
            return False
        ...
        if task.recurrence_count:
            # Count existing instances
            instance_count = (
                db.query(Task).filter(Task.recurrence_parent_id == task_id).count()
            )
            if instance_count >= task.recurrence_count:
                return False
        ...
        last_instance = (
            db.query(Task)
            .filter(Task.recurrence_parent_id == task_id)
    ...
    def create_next_instance(db: Session, task_id: str) -> Optional[Task]:
        ...
        next_date = RecurrenceService.calculate_next_occurrence(
            ...
            (
                [int(x) for x in task.recurrence_days_of_week.split(",")]
