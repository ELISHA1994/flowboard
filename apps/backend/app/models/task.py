...
from app.db.models import TaskStatus, TaskPriority, RecurrencePattern


class RecurrenceConfig(BaseModel):
    """Model for task recurrence configuration"""
    pattern: RecurrencePattern = Field(..., description="Recurrence pattern")
    interval: int = Field(default=1, ge=1, le=365, description="Interval for the pattern (e.g., every 2 days)")
    days_of_week: Optional[List[int]] = Field(None, description="Days of week for weekly pattern (0=Mon, 6=Sun)")
    day_of_month: Optional[int] = Field(None, ge=1, le=31, description="Day of month for monthly pattern")
    month_of_year: Optional[int] = Field(None, ge=1, le=12, description="Month for yearly pattern")
    end_date: Optional[datetime] = Field(None, description="When recurrence should end")
    count: Optional[int] = Field(None, ge=1, le=1000, description="Number of occurrences")
    ...
    def validate_pattern_fields(self):
        ...
        if self.pattern == RecurrencePattern.YEARLY and (not self.day_of_month or not self.month_of_year):
            raise ValueError('day_of_month and month_of_year must be provided for yearly pattern')
...
class TaskCreate(TaskBase):
    ...
    project_id: Optional[str] = Field(None, description="Project ID to associate the task with")
    assigned_to_id: Optional[str] = Field(None, description="User ID to assign the task to")
    recurrence: Optional[RecurrenceConfig] = Field(None, description="Recurrence configuration")
...
class TaskShareCreate(BaseModel):
    ...
    permission: str = Field(default="view", description="Permission level: view, edit, or comment")
