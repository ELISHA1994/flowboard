...
class WebhookSubscriptionCreate(BaseModel):
    ...
    name: str = Field(
        ..., min_length=1, max_length=100, description="Name of the webhook"
    )
    ...
    secret: Optional[str] = Field(
        None, min_length=16, max_length=255, description="Secret for HMAC signature"
    )
    project_id: Optional[str] = Field(
        None, description="Optional project ID to limit webhook to specific project"
    )
...
class WebhookTestRequest(BaseModel):
    ...
    secret: Optional[str] = Field(
        None, description="Optional secret for HMAC signature"
    )
