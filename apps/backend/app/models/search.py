...
class TaskSearchRequest(BaseModel):
    ...
    filters: List[SearchFilter] = Field(
        default=[], description="List of filters to apply"
    )
    sort_by: str = Field(default="created_at", description="Field to sort by")
    sort_order: str = Field(default="desc", description="Sort order (asc/desc)")
    include_shared: bool = Field(
        default=True, description="Include tasks shared with user"
    )
    include_assigned: bool = Field(
        default=True, description="Include tasks assigned to user"
    )
    skip: int = Field(default=0, ge=0, description="Number of tasks to skip")
    limit: int = Field(
        default=20, ge=1, le=100, description="Number of tasks to return"
    )
...
class SavedSearchCreate(BaseModel):
    ...
    name: str = Field(
        ..., min_length=1, max_length=100, description="Name of the saved search"
    )
    description: Optional[str] = Field(
        None, max_length=500, description="Description of the search"
    )
...
class BulkOperationRequest(BaseModel):
    ...
    def validate_task_ids(cls, v: List[str]) -> List[str]:
        if len(v) > 100:
            raise ValueError(
                "Cannot perform bulk operations on more than 100 tasks at once"
            )
        return v
