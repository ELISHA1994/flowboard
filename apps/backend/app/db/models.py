...
task_categories = Table(
    ...
    Base.metadata,
    Column(
        "task_id", String, ForeignKey("tasks.id", ondelete="CASCADE"), primary_key=True
    ),
    Column(
        "category_id",
        String,
...
task_tags = Table(
    ...
    Column(
        "tag_id", String, ForeignKey("tags.id", ondelete="CASCADE"), primary_key=True
    ),
)
...
class User(Base):
    ...
    mentions = relationship(
        "CommentMention", back_populates="mentioned_user", cascade="all, delete-orphan"
    )
    uploaded_files = relationship(
        "FileAttachment", back_populates="uploaded_by", cascade="all, delete-orphan"
    )
    saved_searches = relationship(
        "SavedSearch", back_populates="user", cascade="all, delete-orphan"
    )
    time_logs = relationship(
        "TimeLog", back_populates="user", cascade="all, delete-orphan"
    )
    webhook_subscriptions = relationship(
        "WebhookSubscription", back_populates="user", cascade="all, delete-orphan"
    )
    calendar_integrations = relationship(
    ...
    task_reminders = relationship(
        "TaskReminder", back_populates="user", cascade="all, delete-orphan"
    )
...
class Task(Base):
    ...
    recurrence_month_of_year = Column(
        Integer, nullable=True
    )  # For yearly: month number
    ...
    parent_task = relationship(
        "Task", remote_side=[id], backref="subtasks", foreign_keys=[parent_task_id]
    )
    ...
    attachments = relationship(
        "FileAttachment", back_populates="task", cascade="all, delete-orphan"
    )
    ...
    calendar_syncs = relationship(
        "TaskCalendarSync", back_populates="task", cascade="all, delete-orphan"
    )

    # Reminders relationship
    reminders = relationship(
        "TaskReminder", back_populates="task", cascade="all, delete-orphan"
    )
...
class Project(Base):
    ...
    webhook_subscriptions = relationship(
        "WebhookSubscription", back_populates="project", cascade="all, delete-orphan"
    )
...
class TaskShare(Base):
    ...
    permission = Column(
        SQLEnum(TaskSharePermission), default=TaskSharePermission.VIEW, nullable=False
    )
    created_at = Column(DateTime(timezone=True), default=func.now(), nullable=False)
...
class SavedSearch(Base):
    ...
    __table_args__ = (UniqueConstraint("user_id", "name", name="_user_search_name_uc"),)
...
class CalendarIntegration(Base):
    ...
    )  # to_calendar, from_calendar, both
    created_at = Column(DateTime(timezone=True), default=func.now(), nullable=False)
    updated_at = Column(
        DateTime(timezone=True), default=func.now(), onupdate=func.now(), nullable=False
    )
    ...
    task_sync_mappings = relationship(
        "TaskCalendarSync",
...
class TaskCalendarSync(Base):
    ...
    calendar_integration = relationship(
        "CalendarIntegration", back_populates="task_sync_mappings"
    )

    # Unique constraint
    __table_args__ = (
        UniqueConstraint(
            "task_id", "calendar_integration_id", name="_task_calendar_uc"
        ),
    )
...
class NotificationPreference(Base):
    ...
    )  # task_due, task_assigned, comment_mention, etc.
    ...
    __table_args__ = (
        UniqueConstraint(
            "user_id", "notification_type", "channel", name="_user_notification_uc"
        ),
    )
...
class TaskReminder(Base):
    ...
    reminder_type = Column(
        String(20), default="due_date", nullable=False
    )  # due_date, custom
...
class ActivityType(str, enum.Enum):
    """Task activity type enumeration"""
...
class TaskActivity(Base):
    """Model for tracking task activity history and audit trail"""
    ...
    details = Column(
        Text, nullable=True
    )  # JSON string containing activity-specific data

    # Optional fields for common activity data
    old_value = Column(Text, nullable=True)  # Previous value for change activities
    new_value = Column(Text, nullable=True)  # New value for change activities
