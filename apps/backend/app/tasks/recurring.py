...
def process_recurring_tasks(self, db: Session):
    ...
    try:
        ...
        recurring_tasks = db.query(TaskModel).filter(
            TaskModel.is_recurring == True,
            TaskModel.recurrence_pattern.isnot(None)
        ).all()
        ...
        logger.info(f"Processed {processed_count} recurring tasks, queued {created_count} new instances")
        return {
            "success": True,
...
def create_recurring_task_instance(self, db: Session, task_id: str):
    ...
    try:
        ...
        if new_task:
            logger.info(f"Created new recurring task instance {new_task.id} from template {task_id}")
        ...
        logger.error(f"Failed to create recurring task instance for {task_id}: {str(e)}")
        raise self.retry(countdown=120, max_retries=3)


@celery_app.task(bind=True, base=DatabaseTask, queue="recurring")
def update_recurring_task_template(self, db: Session, task_id: str, updates: Dict[str, Any]):
    ...
    try:
        ...
        if apply_to_future:
            ...
            future_instances = db.query(TaskModel).filter(
            ...
            for instance in future_instances:
                for field, value in updates.items():
                    if field not in ["apply_to_future_instances", "id", "created_at", "parent_task_id"]:
            ...
            db.commit()
            logger.info(f"Updated {updated_instances} future instances of recurring task {task_id}")
...
@celery_app.task(bind=True, base=DatabaseTask, queue="recurring")
def cleanup_completed_recurring_instances(self, db: Session, task_id: str, keep_last_n: int = 10):
    ...
    try:
        ...
        completed_instances = db.query(TaskModel).filter(
            TaskModel.parent_task_id == task_id,
        ...
        if len(completed_instances) > keep_last_n:
            ...
            logger.info(f"Cleaned up recurring task {task_id}: deleted {deleted_count}, archived {archived_count}")
        ...
        logger.error(f"Failed to cleanup recurring task instances for {task_id}: {str(e)}")
        raise self.retry(countdown=300, max_retries=3)
        ...
        for task_id in task_ids:
            try:
                if RecurrenceService.should_create_next_instance(db, task_id):
                    ...
                    results.append({"task_id": task_id, "celery_task_id": result.id, "status": "queued"})
                else:
