...
def deliver_webhook(
    self, db: Session, subscription_id: str, event_type: str, payload: Dict[str, Any]
):
    ...
    try:
        ...
        if event_type not in subscription.events:
            logger.debug(
                f"Event type {event_type} not subscribed for {subscription_id}"
            )
        ...
        if 200 <= response.status_code < 300:
            ...
            logger.info(
                f"Webhook delivered successfully to {subscription.url} for event {event_type}"
            )
            ...
            logger.warning(
                f"Webhook delivery failed to {subscription.url}: {response.status_code}"
            )
            ...
            if subscription.failure_count >= 10:
                ...
                logger.warning(
                    f"Disabled webhook subscription {subscription_id} after {subscription.failure_count} failures"
                )
        ...
        logger.error(
            f"Webhook delivery connection error to {subscription.url}: {str(e)}"
        )
        raise self.retry(countdown=60 * (self.request.retries + 1), max_retries=3)
    ...
    try:
        ...
        for subscription in subscriptions:
            try:
                ...
                logger.error(
                    f"Failed to queue webhook delivery for subscription {subscription.id}: {str(e)}"
                )

        logger.info(
            f"Queued {len(delivery_tasks)} webhook deliveries for event {event_type}"
        )
        return {
            "success": True,
        ...
        for delivery in failed_deliveries:
            try:
                ...
                deliver_webhook.delay(
                    delivery.subscription_id, delivery.event_type, delivery.payload
                )
                retry_count += 1

            except Exception as e:
                logger.error(
                    f"Failed to queue retry for delivery {delivery.id}: {str(e)}"
                )
...
def send_task_created_webhook(self, task_data: Dict[str, Any]):
    ...
    return broadcast_webhook_event.delay(
        "task.created", task_data, task_data.get("user_id")
    )
...
def send_task_updated_webhook(self, task_data: Dict[str, Any]):
    ...
    return broadcast_webhook_event.delay(
        "task.updated", task_data, task_data.get("user_id")
    )
...
def send_task_completed_webhook(self, task_data: Dict[str, Any]):
    ...
    return broadcast_webhook_event.delay(
        "task.completed", task_data, task_data.get("user_id")
    )
...
def send_task_deleted_webhook(self, task_data: Dict[str, Any]):
    ...
    return broadcast_webhook_event.delay(
        "task.deleted", task_data, task_data.get("user_id")
    )
...
def send_project_created_webhook(self, project_data: Dict[str, Any]):
    ...
    return broadcast_webhook_event.delay(
        "project.created", project_data, project_data.get("owner_id")
    )
...
def send_user_invited_webhook(self, invitation_data: Dict[str, Any]):
    ...
    return broadcast_webhook_event.delay(
        "project.user_invited", invitation_data, invitation_data.get("inviter_id")
    )
