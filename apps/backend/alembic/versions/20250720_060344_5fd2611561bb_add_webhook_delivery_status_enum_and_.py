...
def upgrade() -> None:
    ...
    try:
        ...
        webhook_status_enum = sa.Enum('PENDING', 'DELIVERED', 'FAILED', name='webhookdeliverystatus')
        webhook_status_enum.create(op.get_bind())
        
        op.add_column('webhook_deliveries', sa.Column('status', webhook_status_enum, nullable=False))
        op.add_column('webhook_deliveries', sa.Column('response_status_code', sa.Integer(), nullable=True))
        op.add_column('webhook_deliveries', sa.Column('response_headers', sa.Text(), nullable=True))
        op.add_column('webhook_deliveries', sa.Column('response_body', sa.Text(), nullable=True))
        op.add_column('webhook_deliveries', sa.Column('failure_reason', sa.Text(), nullable=True))
        op.add_column('webhook_deliveries', sa.Column('created_at', sa.DateTime(timezone=True), nullable=False))
...
def downgrade() -> None:
    ...
    try:
        # ### commands auto generated by Alembic - please adjust! ###
        op.add_column('webhook_deliveries', sa.Column('status_code', sa.INTEGER(), autoincrement=False, nullable=True))
        op.add_column('webhook_deliveries', sa.Column('error', sa.TEXT(), autoincrement=False, nullable=True))
        op.add_column('webhook_deliveries', sa.Column('response', sa.TEXT(), autoincrement=False, nullable=True))
        op.add_column('webhook_deliveries', sa.Column('next_retry_at', postgresql.TIMESTAMP(timezone=True), autoincrement=False, nullable=True))
        op.alter_column('webhook_deliveries', 'delivered_at',
                   existing_type=postgresql.TIMESTAMP(timezone=True),
                   nullable=False)
