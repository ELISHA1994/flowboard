...
class MigrationManager:
    ...
    def create_backup(self) -> Path:
        ...
        if not settings.DATABASE_URL.startswith("sqlite"):
            logger.warning(
                "Automatic backup only supported for SQLite. Please backup manually."
            )
            return None
        ...
        backup_name = (
            f"backup_{timestamp}_{self.get_current_revision().replace(' ', '_')}.db"
        )
    ...
    def show_status(self):
        ...
        try:
            result = subprocess.run(
                ["alembic", "history", "-r", "current:"], capture_output=True, text=True
            )
            if result.stdout:
                print("\nPending migrations:")
                print(result.stdout)
    ...
    def create_migration(self, message: str):
        ...
        try:
            ...
            result = subprocess.run(
                ["alembic", "show", "head"], capture_output=True, text=True
            )
            print(f"\nGenerated migration:\n{result.stdout}")
        except Exception as e:
...
def main():
    ...
    parser.add_argument(
        "command",
        choices=["status", "upgrade", "downgrade", "test", "backup", "create"],
