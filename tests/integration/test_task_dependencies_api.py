...
from app.db.models import User, Task, TaskStatus, TaskPriority, TaskDependency
...
class TestTaskDependenciesAPI:
    ...
    def test_create_task_with_parent(self, test_client: TestClient, auth_headers: dict, parent_task: Task):
    ...
    def test_update_task_parent(self, test_client: TestClient, auth_headers: dict, 
                               parent_task: Task, child_tasks: list):
    ...
    def test_remove_task_parent(self, test_client: TestClient, auth_headers: dict, child_tasks: list):
    ...
    def test_prevent_self_parent(self, test_client: TestClient, auth_headers: dict, parent_task: Task):
    ...
    def test_prevent_circular_parent_hierarchy(self, test_client: TestClient, auth_headers: dict, 
                                             parent_task: Task, child_tasks: list):
    ...
    def test_create_task_dependency(self, test_client: TestClient, auth_headers: dict, 
    ...
    def test_prevent_self_dependency(self, test_client: TestClient, auth_headers: dict, 
                                   independent_tasks: list):
    ...
    def test_prevent_duplicate_dependency(self, test_client: TestClient, auth_headers: dict, 
                                        independent_tasks: list, test_db: Session):
        ...
        assert response.status_code == 400  # ConflictException is mapped to 400 in our error handler
        assert "already exists" in response.json()["message"]
    
    def test_prevent_circular_dependency(self, test_client: TestClient, auth_headers: dict, 
                                       independent_tasks: list, test_db: Session):
        """Test that circular dependencies are prevented"""
        task_a, task_b, task_c = independent_tasks[0], independent_tasks[1], independent_tasks[2]
        
        # Create chain: A -> B -> C
        dep1 = TaskDependency(id=str(uuid.uuid4()), task_id=task_a.id, depends_on_id=task_b.id)
        dep2 = TaskDependency(id=str(uuid.uuid4()), task_id=task_b.id, depends_on_id=task_c.id)
    ...
    def test_get_task_dependencies(self, test_client: TestClient, auth_headers: dict, 
                                  independent_tasks: list, test_db: Session):
        ...
        dep1 = TaskDependency(id=str(uuid.uuid4()), task_id=task_a.id, depends_on_id=task_b.id)
        dep2 = TaskDependency(id=str(uuid.uuid4()), task_id=task_a.id, depends_on_id=task_c.id)
        test_db.add_all([dep1, dep2])
        test_db.commit()
        
        response = test_client.get(f"/tasks/{task_a.id}/dependencies", headers=auth_headers)
    ...
    def test_delete_task_dependency(self, test_client: TestClient, auth_headers: dict, 
                                   independent_tasks: list, test_db: Session):
        ...
        response = test_client.delete(
            f"/tasks/{task_a.id}/dependencies/{dep.id}", 
            headers=auth_headers
        )
    ...
    def test_check_task_completion_readiness(self, test_client: TestClient, auth_headers: dict, 
                                            independent_tasks: list, test_db: Session):
        ...
        dep1 = TaskDependency(id=str(uuid.uuid4()), task_id=task_a.id, depends_on_id=task_b.id)
        dep2 = TaskDependency(id=str(uuid.uuid4()), task_id=task_a.id, depends_on_id=task_d.id)
        test_db.add_all([dep1, dep2])
        test_db.commit()
        
        response = test_client.get(
            f"/tasks/{task_a.id}/can-complete", 
            headers=auth_headers
        )
    ...
    def test_get_task_subtasks(self, test_client: TestClient, auth_headers: dict, 
                              parent_task: Task, child_tasks: list):
        """Test getting subtasks of a parent task"""
        response = test_client.get(f"/tasks/{parent_task.id}/subtasks", headers=auth_headers)
    ...
    def test_parent_task_status_update(self, test_client: TestClient, auth_headers: dict, 
                                     parent_task: Task, child_tasks: list, test_db: Session):
        ...
        # Check parent status
        parent_response = test_client.get(f"/tasks/{parent_task.id}", headers=auth_headers)
        assert parent_response.status_code == 200
        parent_data = parent_response.json()
        assert parent_data["status"] == "done"
    
    def test_create_task_with_dependencies(self, test_client: TestClient, auth_headers: dict, 
                                         independent_tasks: list):
        ...
        deps_response = test_client.get(
            f"/tasks/{new_task_id}/dependencies", 
            headers=auth_headers
        )
    ...
    def test_update_task_dependencies(self, test_client: TestClient, auth_headers: dict, 
                                    independent_tasks: list):
        ...
        update_data = {
            "depends_on_ids": [task_c.id, task_d.id]
        }
    ...
    def test_task_not_found_errors(self, test_client: TestClient, auth_headers: dict):
        ...
        response = test_client.get(f"/tasks/{fake_id}/dependencies", headers=auth_headers)
    ...
    def test_unauthorized_access(self, test_client: TestClient, independent_tasks: list):
