...
from app.db.models import User, Task, Category, Tag, TaskStatus, TaskPriority
import uuid
...
class TestTaskCategoriesAndTags:
    ...
    def test_create_task_with_categories_and_tags(self, test_client: TestClient, test_user: User, auth_headers: dict, test_db: Session):
    ...
    def test_update_task_categories(self, test_client: TestClient, test_user: User, auth_headers: dict, test_db: Session):
        ...
        # Update categories
        update_data = {
            "category_ids": [cat2.id]  # Replace with Personal category
        }
        
        response = test_client.put(f"/tasks/{task.id}/categories", json=update_data, headers=auth_headers)
    ...
    def test_update_task_tags(self, test_client: TestClient, test_user: User, auth_headers: dict, test_db: Session):
        ...
        initial_tag = Tag(id=str(uuid.uuid4()), name="old-tag", color="#000000", user_id=test_user.id)
        ...
        # Update tags
        update_data = {
            "tag_names": ["new-tag", "urgent"]
        }
        
        response = test_client.put(f"/tasks/{task.id}/tags", json=update_data, headers=auth_headers)
    ...
    def test_add_tags_to_task(self, test_client: TestClient, test_user: User, auth_headers: dict, test_db: Session):
        ...
        existing_tag = Tag(id=str(uuid.uuid4()), name="existing", color="#000000", user_id=test_user.id)
        ...
        response = test_client.post(f"/tasks/{task.id}/tags", json=add_data, headers=auth_headers)
    ...
    def test_remove_tags_from_task(self, test_client: TestClient, test_user: User, auth_headers: dict, test_db: Session):
        ...
        tags = [
            Tag(id=str(uuid.uuid4()), name="keep", color="#000000", user_id=test_user.id),
            Tag(id=str(uuid.uuid4()), name="remove1", color="#FF0000", user_id=test_user.id),
            Tag(id=str(uuid.uuid4()), name="remove2", color="#00FF00", user_id=test_user.id)
        ]
    ...
    def test_filter_tasks_by_category(self, test_client: TestClient, test_user: User, auth_headers: dict, test_db: Session):
        """Test filtering tasks by category"""
        # Create categories
        work_cat = Category(id=str(uuid.uuid4()), name="Work", user_id=test_user.id)
        personal_cat = Category(id=str(uuid.uuid4()), name="Personal", user_id=test_user.id)
        ...
        response = test_client.get(f"/tasks/?category_id={work_cat.id}", headers=auth_headers)
    ...
    def test_filter_tasks_by_tag(self, test_client: TestClient, test_user: User, auth_headers: dict, test_db: Session):
        """Test filtering tasks by tag"""
        # Create tags
        urgent_tag = Tag(id=str(uuid.uuid4()), name="urgent", color="#FF0000", user_id=test_user.id)
        backend_tag = Tag(id=str(uuid.uuid4()), name="backend", color="#00FF00", user_id=test_user.id)
    ...
    def test_task_response_includes_categories_and_tags(self, test_client: TestClient, test_user: User, auth_headers: dict, test_db: Session):
        """Test that task responses always include categories and tags"""
        # Create task with category and tag
        category = Category(id=str(uuid.uuid4()), name="Work", color="#FF5733", user_id=test_user.id)
        tag = Tag(id=str(uuid.uuid4()), name="urgent", color="#FF0000", user_id=test_user.id)
