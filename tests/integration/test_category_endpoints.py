...
class TestCategoryEndpoints:
    ...
    def test_create_category(self, test_client: TestClient, test_user: User, auth_headers: dict):
    ...
    def test_create_category_duplicate_name(self, test_client: TestClient, test_user: User, auth_headers: dict, test_db: Session):
    ...
    def test_list_categories(self, test_client: TestClient, test_user: User, auth_headers: dict, test_db: Session):
        ...
        categories = [
            Category(id=str(uuid.uuid4()), name="Work", user_id=test_user.id, is_active=True),
            Category(id=str(uuid.uuid4()), name="Personal", user_id=test_user.id, is_active=True),
            Category(id=str(uuid.uuid4()), name="Archive", user_id=test_user.id, is_active=False)
        ]
        ...
        response = test_client.get("/categories/?include_inactive=true", headers=auth_headers)
        
        assert response.status_code == 200
        data = response.json()
        assert len(data) == 3  # All categories
    
    def test_get_category(self, test_client: TestClient, test_user: User, auth_headers: dict, test_db: Session):
    ...
    def test_update_category(self, test_client: TestClient, test_user: User, auth_headers: dict, test_db: Session):
        ...
        response = test_client.put(f"/categories/{category.id}", json=update_data, headers=auth_headers)
    ...
    def test_delete_category(self, test_client: TestClient, test_user: User, auth_headers: dict, test_db: Session):
    ...
    def test_category_isolation(self, test_client: TestClient, test_user: User, auth_headers: dict, test_db: Session):
        ...
        response = test_client.get(f"/categories/{other_category.id}", headers=auth_headers)
