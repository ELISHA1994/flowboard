...
from app.models.project import (
    ProjectCreate, ProjectUpdate, ProjectInvitationCreate,
...
class TestProjectService:
    ...
    def test_get_user_projects_active_only(self, mock_db):
        ...
        # Act
        result = ProjectService.get_user_projects(mock_db, user_id, include_inactive=False)
    ...
    def test_create_invitation_success(self, mock_db):
        ...
        mock_query = Mock()
        mock_query.filter.return_value = mock_query
        mock_query.first.side_effect = [None, None]  # No existing user, no existing invitation
        mock_db.query.return_value = mock_query
    ...
    def test_create_invitation_user_already_member(self, mock_db):
        ...
        # Act & Assert
        with pytest.raises(ValueError, match="User is already a project member"):
            ProjectService.create_invitation(mock_db, project_id, inviter_id, invitation_data)
    ...
    def test_to_detail_response(self, mock_db):
        ...
        project.description = "Test description"
        project.owner_id = user_id
        project.owner = Mock(spec=User, id=user_id, username="owner", email="owner@example.com")
        project.is_active = True
        project.created_at = datetime.now(timezone.utc)
        project.updated_at = datetime.now(timezone.utc)
        project.tasks = []
        ...
        member_mock.id = "m1"
        member_mock.user = Mock(spec=User, id="member123", username="member", email="member@example.com")
