...
from app.services.search_service import SearchService, TaskSearchQuery, TaskSearchFilter, SearchOperator
from app.db.models import Task, TaskStatus, TaskPriority, Category, Tag, User, Project
...
class TestSearchService:
    ...
    def test_task_search_query_builder(self):
        ...
        filter2 = TaskSearchFilter("priority", SearchOperator.GREATER_THAN_OR_EQUAL, TaskPriority.HIGH)
    ...
    def test_apply_filter_operators(self):
        ...
        mock_query.reset_mock()
        mock_query.filter.return_value = mock_query
        filter_in = TaskSearchFilter("status", SearchOperator.IN, ["todo", "in_progress"])
        SearchService.FIELD_MAPPING = {"status": mock_field}  # Re-set mapping after reset
    ...
    def test_get_suggested_filters(self):
        ...
        with patch.object(SearchService, 'get_suggested_filters', return_value=expected_response):
