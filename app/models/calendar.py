...
class CalendarIntegrationCreate(BaseModel):
    ...
    calendar_id: str = Field(..., max_length=255, description="Calendar ID from provider")
    calendar_name: str = Field(..., max_length=255, description="Human-readable calendar name")
    access_token: str = Field(..., description="OAuth access token")
    refresh_token: Optional[str] = Field(None, description="OAuth refresh token")
    token_expires_at: Optional[datetime] = Field(None, description="Token expiration time")
    sync_direction: SyncDirectionEnum = Field(default=SyncDirectionEnum.BOTH, description="Sync direction")
...
class CalendarSyncRequest(BaseModel):
    ...
    integration_id: Optional[str] = Field(None, description="Specific integration to sync")
