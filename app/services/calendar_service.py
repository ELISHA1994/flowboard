...
from app.db.models import CalendarIntegration, TaskCalendarSync, Task
...
class CalendarService:
    ...
    def sync_task_to_calendar(
        ...
        existing_sync = db.query(TaskCalendarSync).filter(
    ...
    def get_task_syncs(
        db: Session,
        task_id: str
    ) -> List[TaskCalendarSync]:
        """Get all calendar syncs for a task."""
        return db.query(TaskCalendarSync).filter(
            TaskCalendarSync.task_id == task_id
        ).all()
    
    @staticmethod
    def remove_task_sync(
        db: Session,
        task_id: str,
        integration_id: str
    ) -> bool:
        """Remove a task calendar sync."""
        sync = db.query(TaskCalendarSync).filter(
            TaskCalendarSync.task_id == task_id,
            TaskCalendarSync.calendar_integration_id == integration_id
        ).first()
    ...
    async def sync_with_google_calendar(
        ...
        event_data = {
            ...
            "end": {
                "dateTime": (task.due_date + timedelta(hours=1)).isoformat() if task.due_date else (datetime.now(timezone.utc) + timedelta(hours=1)).isoformat(),
                "timeZone": "UTC"
            },
            "reminders": {
                "useDefault": False,
        ...
        if hasattr(task, 'reminder_date') and task.reminder_date:
            minutes_before = int((task.due_date - task.reminder_date).total_seconds() / 60) if task.due_date else 60
            event_data["reminders"]["overrides"].append({
                "method": "popup",
                "minutes": max(0, minutes_before)
            })
        ...
        try:
            async with httpx.AsyncClient() as client:
                ...
                if response.status_code in [200, 201]:
                    ...
                    logger.error(f"Google Calendar sync failed: {response.status_code} - {response.text}")
                    return None
        ...
        try:
            async with httpx.AsyncClient() as client:
                ...
                if response.status_code in [200, 201]:
                    ...
                    logger.error(f"Microsoft Calendar sync failed: {response.status_code} - {response.text}")
                    return None
    ...
    async def sync_task(
        ...
        if action in ["create", "update"] and integration.sync_direction == SyncDirection.FROM_CALENDAR:
            return False
        elif action == "delete" and integration.sync_direction == SyncDirection.FROM_CALENDAR:
            return False
    ...
    async def sync_all_tasks(
        ...
        if not integrations:
            return {"synced": 0, "failed": 0, "errors": ["No active calendar integrations found"]}
