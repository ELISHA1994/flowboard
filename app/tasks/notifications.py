...
from app.db.models import Notification, User, Task as TaskModel
...
def send_email_notification(self, recipient_email: str, subject: str, content: str, notification_type: str = "info"):
...
def send_task_reminder_notification(self, db: Session, task_id: str, user_id: str):
    ...
    try:
        ...
        if not task or not user:
            logger.warning(f"Missing data for task reminder: task={task_id}, user={user_id}")
            return {"success": False, "reason": "Missing data"}
        ...
        logger.info(f"Task reminder notification sent for task {task_id} to user {user_id}")
        return {"success": True, "notification_id": notification.id}
...
@celery_app.task(bind=True, base=DatabaseTask, queue="notifications")
def send_project_invitation_notification(self, db: Session, project_id: str, invited_user_id: str, inviter_id: str):
    ...
    try:
        ...
        if not project or not invited_user or not inviter:
            logger.warning(f"Missing data for project invitation: project={project_id}, invited_user={invited_user_id}, inviter={inviter_id}")
            return {"success": False, "reason": "Missing data"}
        ...
        send_email_notification.delay(invited_user.email, subject, content, "project_invitation")
        
        logger.info(f"Project invitation notification sent for project {project_id} to user {invited_user_id}")
        return {"success": True, "notification_id": notification.id}
