...
from app.db.models import User, Task as TaskModel, Project
...
def precompute_analytics(self, db: Session):
    ...
    try:
        ...
        active_users = db.query(User).join(TaskModel).filter(
            TaskModel.created_at >= thirty_days_ago
        ).distinct().all()
        ...
        for user in active_users:
            try:
                ...
                logger.error(f"Error precomputing analytics for user {user.id}: {str(e)}")
                continue
        
        logger.info(f"Analytics precomputation completed: {processed_count} users processed, {cache_hits} cache hits")
        return {
            "success": True,
...
@celery_app.task(bind=True, base=DatabaseTask, queue="analytics")
def compute_user_analytics(self, db: Session, user_id: str, force_refresh: bool = False):
...
def compute_project_analytics(self, db: Session, project_id: str, user_id: str):
    ...
    try:
        ...
        if not project.has_permission(user_id, "VIEWER"):
            logger.warning(f"User {user_id} does not have permission to view project {project_id} analytics")
            return {"success": False, "reason": "Permission denied"}
        
        computed_metrics = []
        
        # Project task statistics
        cache_key = f"{settings.CACHE_PREFIX_ANALYTICS}project_stats:{project_id}:{user_id}"
        ...
        categories = AnalyticsService.get_category_distribution(db, user_id, project_id=project_id)
        cache_service.set(cache_key, categories, ttl=1800)
        computed_metrics.append("project_category_distribution")
        
        # Project tag distribution
        cache_key = f"{settings.CACHE_PREFIX_ANALYTICS}project_tags:{project_id}:{user_id}"
        ...
        logger.info(f"Computed project analytics for project {project_id}: {computed_metrics}")
        return {
            "success": True,
...
@celery_app.task(bind=True, base=DatabaseTask, queue="analytics")
def generate_time_tracking_report(self, db: Session, user_id: str, start_date: str, end_date: str, group_by: str = "task"):
    ...
    try:
        ...
        logger.error(f"Failed to generate time tracking report for user {user_id}: {str(e)}")
        raise self.retry(countdown=120, max_retries=3)


@celery_app.task(bind=True, base=DatabaseTask, queue="analytics")
def export_tasks_async(self, db: Session, user_id: str, task_ids: Optional[List[str]], format: str = "csv"):
...
def compute_system_wide_analytics(self, db: Session):
    ...
    try:
        ...
        active_projects = db.query(Project).join(TaskModel).filter(
            TaskModel.created_at >= thirty_days_ago
        ).distinct().count()
        ...
        system_metrics = {
            ...
            "tasks": {
                ...
                "average_per_user": (total_tasks / total_users) if total_users > 0 else 0
            },
            "projects": {
                "total": total_projects,
                "active_30_days": active_projects,
                "activity_rate": (active_projects / total_projects * 100) if total_projects > 0 else 0
            }
        }
...
def invalidate_user_analytics_cache(self, db: Session, user_id: str):
    ...
    try:
        ...
        logger.info(f"Invalidated {invalidated_count} analytics cache entries for user {user_id}")
        return {"success": True, "invalidated_count": invalidated_count}
        
    except Exception as e:
        logger.error(f"Failed to invalidate analytics cache for user {user_id}: {str(e)}")
